...
use risk_stream::{
    enriched_topic, extract_subject_ref, topic_to_domain, EnrichmentRequest, EnrichmentResponse,
    ProcessorError, RiskEvent,
};
use std::{
    net::SocketAddr,
    sync::{
...
async fn enrich_event(
    ...
    if !resp.status().is_success() {
        ...
        return Err(ProcessorError::EnrichmentError(format!(
            "ML service {} — {}",
            status, body
        )));
    }
...
async fn publish_to_dapr(
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(ProcessorError::DaprError(format!(
            "Dapr {} — {}",
            status, body
        )));
    }
...
// ─── Event Processing ─────────────────────────────────────────────────────────
async fn process_message(state: &AppState, topic: &str, payload_bytes: &[u8]) {
    ...
    let event = RiskEvent {
        ...
        subject_name: raw
            .get("subjectName")
            .or_else(|| raw.get("subject_name"))
            .and_then(|v| v.as_str())
            .map(String::from),
        risk_score,
        risk_tier: raw
            .get("riskTier")
            .or_else(|| raw.get("risk_tier"))
            .and_then(|v| v.as_str())
            .map(String::from),
        tenant_id,
        payload: raw.clone(),
    ...
    let enrichment = match enrich_event(state, &event).await {
        ...
        Err(e) => {
            warn!(
                "[RiskStream] ML enrichment failed for {}: {}",
                subject_ref, e
            );
            None
        }
    ...
    match publish_to_dapr(state, out_topic, enriched_payload).await {
        ...
        Err(e) => {
            warn!(
                "[RiskStream] Dapr publish failed for {}: {}",
                subject_ref, e
            );
            EVENTS_FAILED.fetch_add(1, Ordering::Relaxed);
        }
...
async fn run_consumer(state: AppState, brokers: String, group_id: String) {
    let topics = vec![
        "bis.aml.alerts",
        "bis.aml.events",
        "bis.transaction.events",
        "bis.kyc.events",
        "bis.investigation.events",
        "bis.case.events",
        "bis.screening.events",
        "bis.criminal_records.events",
        "bis.sar.events",
        "bis.goaml.events",
        "bis.risk_profile.events",
        "bis.insider.events",
        "bis.biometric.events",
        "bis.field_visit.events",
        "bis.lex.events",
        "bis.payment.events",
        "bis.stablecoin.events",
        "bis.mojaloop.events",
    ];
    ...
    info!(
        "[RiskStream] Subscribed to {} topics on {}",
        topics.len(),
        brokers
    );
...
async fn service_auth(headers: HeaderMap, request: axum::extract::Request, next: Next) -> Response {
    let expected = std::env::var("BIS_RISK_STREAM_KEY").unwrap_or_default();
...
async fn main() {
    ...
    let kafka_brokers = match std::env::var("KAFKA_BROKERS") {
        ...
        _ => {
            error!("KAFKA_BROKERS must be configured");
            return;
        }
    };
    let group_id =
    ...
    let dapr_pubsub = match std::env::var("DAPR_PUBSUB_NAME") {
        Ok(value) if !value.trim().is_empty() => value,
        _ => {
            error!("DAPR_PUBSUB_NAME must be configured");
            return;
        }
    };
    if std::env::var("BIS_RISK_STREAM_KEY")
        .unwrap_or_default()
        .is_empty()
    {
        error!("BIS_RISK_STREAM_KEY must be configured");
        return;
    }
