...
mod tests {
    ...
    fn test_topic_routing() {
        assert_eq!(topic_to_domain("bis.aml.alerts"), Some(EventDomain::AmlAlert));
        assert_eq!(topic_to_domain("bis.transaction.events"), Some(EventDomain::Transaction));
        assert_eq!(topic_to_domain("bis.sar.events"), Some(EventDomain::SarFiling));
    ...
    fn test_enriched_topic() {
        ...
        assert_eq!(enriched_topic(&EventDomain::Transaction), "bis.transaction.enriched");
    }

    #[test]
    fn test_extract_subject_ref() {
        let payload = serde_json::json!({ "alertRef": "AML-123", "amount": 5000 });
        assert_eq!(extract_subject_ref(&EventDomain::AmlAlert, &payload), "AML-123");
    }
}
