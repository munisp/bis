/**
 * goAML STR Reports tab — BIS Mobile (Expo)
 * Suspicious Transaction Reports with full CRUD and submission workflow.
 */
...
export default function GoAMLScreen() {
  ...
  const [form, setForm] = useState({ reportType: "STR", subject: "", description: "", amount: "" });

  const utils = trpc.useUtils();

  const { data, isLoading, refetch, isFetching } = trpc.goaml.list.useQuery({
    search,
    page,
    limit: 20,
  });
  ...
  const createMutation = trpc.goaml.create.useMutation({
    onSuccess: () => {
      ...
      setForm({ reportType: "STR", subject: "", description: "", amount: "" });
  ...
  const submitMutation = trpc.goaml.submit.useMutation({
    onSuccess: () => {
      utils.goaml.list.invalidate();
      Alert.alert("Submitted", "Report submitted to goAML");
    },
    onError: (err) => Alert.alert("Error", err.message),
  });

  const deleteMutation = trpc.goaml.delete.useMutation({
    onSuccess: () => utils.goaml.list.invalidate(),
    onError: (err) => Alert.alert("Error", err.message),
  });
    ...
    return (
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <Text style={styles.reportRef}>{(r.reportRef as string) ?? "—"}</Text>
        ...
        <Text style={styles.detail}>Type: {(r.reportType as string) ?? "STR"}</Text>
        <Text style={styles.detail}>Subject: {(r.subject as string) ?? "—"}</Text>
        {r.amount && (
          <Text style={styles.detail}>Amount: {(r.currency as string) ?? "NGN"} {Number(r.amount).toLocaleString()}</Text>
        )}
        ...
        <View style={styles.actions}>
          {status === "draft" && (
            <TouchableOpacity
              ...
              onPress={() =>
                Alert.alert("Submit", "Submit this report to goAML?", [
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>goAML Reports</Text>
          <Text style={styles.subtitle}>{total} total reports</Text>
        </View>
        <TouchableOpacity style={styles.createBtn} onPress={() => setShowCreateModal(true)}>
          <Text style={styles.createBtnText}>+ New STR</Text>
        </TouchableOpacity>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No goAML reports found</Text>
          }
          onEndReached={() => {
      ...
      <Modal visible={showCreateModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>New STR Report</Text>
            <TextInput
              style={styles.input}
            ...
            <TextInput
              style={styles.input}
              placeholder="Suspicious amount (NGN)"
              placeholderTextColor="#64748b"
              value={form.amount}
...
const styles = StyleSheet.create({
  ...
  reportRef: { fontSize: 14, fontWeight: "600", color: "#f8fafc" },
