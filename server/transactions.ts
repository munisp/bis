...
function quickScore(data: {
  ...
  if (HIGH_RISK_COUNTRIES.has(data.originatorCountry)) {
    score += 35;
    flags.push("high_risk_originator_country");
  }
  if (HIGH_RISK_COUNTRIES.has(data.beneficiaryCountry)) {
    score += 35;
    flags.push("high_risk_beneficiary_country");
  }

  const threshold = CTR_THRESHOLDS[data.currency] ?? 10_000;
  const lower = threshold * 0.95;
  if (data.amount >= lower && data.amount < threshold) {
    score += 45;
    flags.push("potential_structuring");
  }
  if (data.amount >= 100_000 && data.amount % 10_000 === 0) {
    score += 10;
  ...
  if (suspiciousKw.some(kw => narration.includes(kw))) {
    score += 30;
    flags.push("suspicious_narration");
  }
...
export const transactionsRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (input.riskLevel)
        conditions.push(eq(transactions.amlRiskLevel, input.riskLevel as any));
      if (input.currency)
        conditions.push(eq(transactions.currency, input.currency));
      if (input.minAmount !== undefined)
        conditions.push(gte(transactions.amount, input.minAmount));
      if (input.maxAmount !== undefined)
        conditions.push(lte(transactions.amount, input.maxAmount));
      if (input.fromDate)
        conditions.push(gte(transactions.valueDate, new Date(input.fromDate)));
      if (input.toDate)
        conditions.push(lte(transactions.valueDate, new Date(input.toDate)));
      if (ctx.tenantId !== null)
        conditions.push(eq(transactions.tenantId, ctx.tenantId));
      if (input.search) {
        conditions.push(
          or(
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        const [inserted] = await db
          ...
          .returning();
        if (!inserted) throw new Error("Transaction insert returned no record");
        tx = inserted;
      } catch (error) {
  ...
  update: adminProcedure
    .input(
      z.object({
        id: z.number(),
        status: z
          .enum(["pending", "failed", "flagged", "blocked", "under_review"])
          .optional(),
        narration: z.string().optional(),
      })
    )
    .mutation(async ({ input, ctx }) => {
      ...
      if (existing.status === "completed" || existing.status === "reversed") {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "Settled transactions must use the authoritative reversal workflow",
        });
      }
      const [tx] = await db
        .update(transactions)
  ...
  listAlerts: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const conditions = [];
      if (input.status)
        conditions.push(eq(amlAlerts.status, input.status as any));
      if (input.riskLevel)
        conditions.push(eq(amlAlerts.riskLevel, input.riskLevel as any));
      if (ctx.tenantId !== null)
        conditions.push(eq(amlAlerts.tenantId, ctx.tenantId));
      const where = conditions.length > 0 ? and(...conditions) : undefined;
      const [rows, [{ total }]] = await Promise.all([
        db
          .select()
  ...
  resolveAlert: writeProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [alert] = await db
        ...
        .where(
          and(
            eq(amlAlerts.id, input.alertId),
            ...(ctx.tenantId !== null
              ? [eq(amlAlerts.tenantId, ctx.tenantId)]
              : [])
          )
        )
        .returning();
  ...
  stats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [alertStats] = await db
      ...
      .from(amlAlerts)
      .where(
        ctx.tenantId !== null ? eq(amlAlerts.tenantId, ctx.tenantId) : undefined
      );
