 ...
 * metric value, logs every evaluation to rule_evaluations, and auto-creates
 * alerts + optional owner notifications when thresholds are breached.
 */

import { getDb } from "./db";
import { alertRules, alerts, auditLog, ruleEvaluations } from "../drizzle/schema";
 ...
 * Logs every evaluation (triggered or not) to rule_evaluations.
...
export async function evaluateAlertRules(
  ...
  for (const rule of rules) {
    ...
    // Log every evaluation (triggered or not) to rule_evaluations
    try {
      await db.insert(ruleEvaluations).values({
        ...
        triggered,
        alertCreated,
        context: ctx.triggeredBy ?? null,
      });
    } catch (e) {
      console.warn("[AlertRules] Failed to log evaluation for rule", rule.id, e);
    }
  }
