...
| FOF-07 | Critical | Payment-rail initiation invoked the external rail before a durable unique transaction claim. Concurrent requests with the same reference could both submit money-moving rail calls before the database uniqueness violation surfaced. | The router now inserts a tenant-scoped, unique pre-rail claim before calling the rail, returns the existing claim after a duplicate-key race, enforces Naira precision and maximum value, and rejects self-transfers. | `server/paymentRails.ts`; `server/paymentRails.initiationGuard.test.ts` |
| FOF-08 | High | The core transaction intake path looked up an idempotency key without tenant scope, did not recover safely from a concurrent unique-key race, and created AML alerts without the originating tenant. | Existing idempotency records are tenant-scoped; duplicate-key races re-read only the caller’s tenant record; generated AML alerts now persist tenant ownership; alert list, resolution, and statistics enforce the same scope. | `server/transactions.ts`; `server/transactions.idempotencyGuard.test.ts` |
| FOF-09 | High | Investigation debit transfers used random identifiers and persisted reconciliation after the ledger call, allowing retry-driven duplicate debit attempts and leaving post-ledger persistence ambiguous. | Debit IDs are now deterministic by tenant, investigation, tier, and amount; a unique reconciliation claim is persisted before ledger execution and marked reconciled only afterward. | `server/billing.ts`; `server/billing.test.ts` |
